///same as challenge19 - break bunch of ciphertexts generated using fixed nonce CTR
pub fn break_fixed_nonce_ctr_statistically(ciphers: Vec<Vec<u8>>) -> Vec<String> {
        ...
        for byte in 0..255 {
            ...
            for i in 0..ciphers.len() {
                if index < ciphers[i].len() {
                    ...
                    if xor_byte < 32 || xor_byte >= 123 || (xor_byte >= 35 && xor_byte <= 38) {
                        score -= 100;
                    }
                    ...
                    if xor_byte >= 60 && xor_byte <= 63 {
                        score -= 10;
                    }
                    if xor_byte >= 91 && xor_byte <= 96 {
                        score -= 100;
                    }
                }
            }
